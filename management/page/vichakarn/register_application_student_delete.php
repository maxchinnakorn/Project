                        ...
                        <div class="section-seperator margin-b-50">
                            <div class="margin-b-50">
                                <div class="margin-b-30">
                                    <p style="margin-bottom:25px;"><h2 style="font-family: 'Itim', cursive; padding-bottom:20px;">ลบทะเบียนนักเรียน</h2></p>
									...
									if($chk == 1)
									{
									$sql2 = "SELECT * FROM `data_student`";
									}
									else
									{
									$sql2 = "SELECT * FROM `data_student` WHERE `student_id` LIKE '%$search%' OR `student_firstname` LIKE '%$search%' OR `student_surname` LIKE '%$search%'";
									}
									$sql = "$sql2";
									...
									<table width="950px" border="0" style="font-family: 'Itim', cursive; font-size:18px">
									 <tbody>
										...
										<? do { ?>
										<tr>
										  <td style="padding:5px 20px 5px 0px;"><?=$objresult['student_id']?></td>
										  <td style="padding:5px 20px 5px 0px;"><?=$objresult['student_firstname']?></td>
										  <td style="padding:5px 20px 5px 0px;"><?=$objresult['student_surname']?></td>
										  <td style="padding:5px 20px 5px 0px;">
											  <div align="center">
											  <?
										  ...
										  <td style="padding:5px 20px 5px 0px;">
											  ...
											  <?
											  	if($objresult['user_status'] == 0)
												{
													echo "<a href='?page=vichakarn&work=register_student_delete&id=".$objresult['student_id']."&status=1'><input type='button' value='   ปิดบัญชี   ' style='background-color:#FF0033; color:white;'></a>";
												}
											    else
												{
													echo "<a href='?page=vichakarn&work=register_student_delete&id=".$objresult['student_id']."&status=0'><input type='button' value='  เปิดบัญชี  ' style='background-color:#33FF00; color:green;'></a>";
												}
											  ?>
										  	  </div>
										  </td>
